...
import { useImportQuotes, type QuoteImportRow } from "@/hooks/useQuotes";
import {
  guessColumnMapping,
...
const FIELD_LABELS: Record<string, string> = {
  ...
  total: "Total amount",
  description: "Description (becomes a single line item)",
  status: "Status",
  date: "Date",
  notes: "Notes",
};
...
export function ImportQuotesDialog() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setDataRows(rest);
    setMapping(guessColumnMapping(headerRow, QUOTE_FIELD_KEYWORDS));
  }
  ...
  function buildRows(): QuoteImportRow[] {
    ...
    return dataRows.map((row) => ({
      ...
      totalCents: totalIdx !== -1 ? parseCurrencyToCents(row[totalIdx]) : null,
      description: descIdx !== -1 ? row[descIdx]?.trim() || null : null,
      status: normalizeQuoteStatus(statusIdx !== -1 ? row[statusIdx] : undefined),
      date: dateIdx !== -1 ? parseDateLoose(row[dateIdx]) : null,
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      const { imported, skipped } = await importQuotes.mutateAsync({ ownerId: user.id, rows });
      toast.success(
        `Imported ${imported} quote${imported === 1 ? "" : "s"}${skipped > 0 ? ` (${skipped} skipped — missing client name)` : ""}`,
      );
      setOpen(false);
      reset();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Import quotes from CSV</DialogTitle>
        </DialogHeader>

        {headers.length === 0 ? (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Export your quotes/estimates to CSV from Jobber (or any other tool) and upload it here.
              Each row becomes one quote with a single line item for the total — you can add itemized
              line items afterward if needed.
            </p>
            <input
              type="file"
              accept=".csv,text/csv"
        ...
        <DialogFooter>
          ...
          <Button onClick={handleImport} disabled={headers.length === 0 || importing}>
            {importing ? "Importing…" : `Import ${dataRows.length || ""} quote${dataRows.length === 1 ? "" : "s"}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
